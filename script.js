(function(){
  ...
  // Corrige/normaliza dados vindos do localStorage (ex.: registros salvos por
  // uma versão anterior do app, sem o campo "categories" ou com outros campos
  // ausentes). Sem isso, telas como "Buscar" quebram ao tentar ler
  // propriedades de um livro incompleto (ex.: b.categories.join(...)).
  function normalizeData(d){
    if(!d || typeof d !== "object") return seedData();

    if(!Array.isArray(d.books)) d.books = [];
    d.books = d.books.filter(function(b){ return b && typeof b === "object"; }).map(function(b){
      return {
        id: b.id || ("b"+Date.now()+Math.random().toString(36).slice(2,7)),
        title: b.title || "Sem título",
        author: b.author || "Autor desconhecido",
        categories: Array.isArray(b.categories) ? b.categories : [],
        status: (b.status === "Emprestado") ? "Emprestado" : "Disponível",
        synopsis: b.synopsis || "",
    ...
    d.loans = d.loans.filter(function(l){ return l && typeof l === "object" && l.id && l.bookId; });

    if(!Array.isArray(d.reservations)) d.reservations = [];
    d.reservations = d.reservations.filter(function(r){ return r && typeof r === "object" && r.id && r.bookId; });
  ...
  function loadData(){
    try{
      ...
      if(raw){
        ...
        saveData(parsed); // persiste a versão já corrigida
        return parsed;
      }
    }catch(e){}
    var seeded = seedData();
  ...
  function render(){
    try{
      ...
      // Rede de segurança: se algum dado salvo no navegador estiver corrompido
      // e mesmo assim escapar da normalização, evita deixar a tela em branco.
  ...
  function recoverFromRenderError(){
    // Primeira tentativa: os dados do acervo podem estar com algum campo
    // inesperado que passou pela normalização — corrige de novo e tenta
    // renderizar a tela atual mais uma vez.
    if(!state.__recovering){
      ...
      try{
        ...
        console.error("Falha na recuperação:", err2);
      }
    }
    // Se ainda assim falhar, evita a tela em branco: limpa os dados salvos
    // e mostra uma mensagem para o usuário reiniciar.
    var root = document.getElementById("app");
    root.innerHTML =
      '<div class="screen"><div class="eyebrow">Ops</div>'+
      '<h1 class="page-title">Algo deu errado ao carregar seus dados</h1>'+
      '<p class="page-lede">Isso costuma acontecer por causa de um dado antigo salvo no navegador. '+
      'Clique no botão abaixo para restaurar o acervo de demonstração e continuar usando o BiblioWeb.</p>'+
      '<button type="button" class="btn btn-primary" id="recover-reset-btn">Restaurar dados de demonstração</button></div>';
    var btn = document.getElementById("recover-reset-btn");
  ...
  function renderBusca(){
    ...
    var results = data.books.filter(function(b){
      ...
      var matchQ = !q || b.title.toLowerCase().indexOf(q)>-1 || b.author.toLowerCase().indexOf(q)>-1 ||
        cats.some(function(c){ return c.toLowerCase().indexOf(q)>-1; });
      // Filtro em conjunto: com mais de uma categoria marcada, só entram livros que têm TODAS elas.
      var matchCat = selected.length===0 || selected.every(function(c){ return cats.indexOf(c)>-1; });
